...
import { heroTemples } from "./heroShowcase";
...
export function HeroSlider() {
  return (
    <section className="hero-slider" aria-label="Featured temples across India">
      ...
      >
        {heroTemples.map((temple) => (
          <SwiperSlide key={temple.id}>
            {({ isActive }) => (
              <div className="hero-slide">
                <picture>
                  <source media="(max-width: 760px)" srcSet={temple.webp960} type="image/webp" />
                ...
                <span className="hero-slide__caption">
                  {temple.name} <span className="hero-slide__dot" aria-hidden="true">•</span> {temple.city}
                </span>
              </div>
            )}
          </SwiperSlide>
